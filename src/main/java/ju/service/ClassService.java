...
public class ClassService {
    ...
    public ClassDao getClassById(Integer departmentId, Integer courseId, Integer id) {
        ...
        return new ClassDao(aClass.getId(),aClass.getStartTime(), aClass.getEndTime(), aClass.getDaysOfWeek(),aClass.getRoomNumber(),
                aClass.getCapacity(),aClass.getSemester(),aClass.getYear(), aClass.getStatus());
    }

    // Get all Classes for a given department
    public List<ClassDao> getClassesInCourse(Integer departmentId, Integer courseId) {
        if (!departmentRepository.existsById(departmentId))
        ...
        return classes.stream()
                .map(c->new ClassDao(c.getId(),c.getStartTime(), c.getEndTime(), c.getDaysOfWeek(),c.getRoomNumber(),
                        c.getCapacity(),c.getSemester(),c.getYear(), c.getStatus()))
                .collect(Collectors.toList());
    }

    // Get all Classes for a given department
    public List<ClassDao> getClassesInDepartment(Integer departmentId) {
        if (!departmentRepository.existsById(departmentId))
            throw new ResourceNotFoundException(String.format("Department not found with ID %d.", departmentId));
