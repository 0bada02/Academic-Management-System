...
public class InstructorController {
    ...
    public ResponseEntity<InstructorDao> getInstructorById(
            @PathVariable Integer departmentId,
            @PathVariable Integer id) {
        return ResponseEntity.ok(instructorService.getInstructorById(departmentId, id));
    }

    // Get an instructor-course by their ID
    @GetMapping("/{departmentId}/instructor-course/{id}")
    public ResponseEntity<ArrayList<CourseDao>> getCoursesForInstructor(
            @PathVariable Integer departmentId,
            @PathVariable Integer id) {
        return ResponseEntity.ok(instructorService.getCoursesForInstructor(departmentId, id));
    }

    // Get an instructor-class by their ID
    @GetMapping("/{departmentId}/instructor-class/{id}")
    public ResponseEntity<ArrayList<ClassDao>> getClassesForInstructor(
            @PathVariable Integer departmentId,
            @PathVariable Integer id) {
        return ResponseEntity.ok(instructorService.getClassesForInstructor(departmentId, id));
    }

    // Get all instructors for a given department
    @GetMapping("/{departmentId}/instructor")
    public ResponseEntity<List<InstructorDao>> getInstructorsInDepartment(@PathVariable Integer departmentId) {
        return ResponseEntity.ok(instructorService.getInstructorsInDepartment(departmentId));
    }

    // Get all instructors for a collage
    @GetMapping("/instructor")
    public ResponseEntity<List<InstructorDao>> getAllInstructors() {
