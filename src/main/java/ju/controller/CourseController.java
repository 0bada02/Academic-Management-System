...
public class CourseController {
    ...
    public ResponseEntity<CourseDao> getCourseById(
            @PathVariable Integer departmentId,
            @PathVariable Integer id) {
        return ResponseEntity.ok(courseService.getCourseById(departmentId, id));
    }

    // Get a course-class by their ID
    @GetMapping("/{departmentId}/course-class/{id}")
    public ResponseEntity<CourseDao> getCourseById_Class(
            @PathVariable Integer departmentId,
            @PathVariable Integer id) {
        return ResponseEntity.ok(courseService.getCourseById_Class(departmentId, id));
    }

    // Get all courses for a given department
    @GetMapping("/{departmentId}/course")
    public ResponseEntity<List<CourseDao>> getCoursesInDepartment(@PathVariable Integer departmentId) {
        return ResponseEntity.ok(courseService.getCoursesInDepartment(departmentId));
    }

    // Get all courses-class for a given department
    @GetMapping("/{departmentId}/course-class")
    public ResponseEntity<List<CourseDao>> getCoursesInDepartment_Class(@PathVariable Integer departmentId) {
        return ResponseEntity.ok(courseService.getCoursesInDepartment_Class(departmentId));
    }
    ...
    public ResponseEntity<List<CourseDao>> getAllCourses_Class() {
        return ResponseEntity.ok(courseService.getAllCourses_Class());
    }
