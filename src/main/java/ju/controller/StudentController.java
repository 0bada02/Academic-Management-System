...
public class StudentController {
    ...
    public ResponseEntity<StudentDao> getStudentById(
            @PathVariable Integer departmentId,
            @PathVariable Integer id) {
        return ResponseEntity.ok(studentService.getStudentById(departmentId, id));
    }

    // Get a student-course by their ID
    @GetMapping("/{departmentId}/student-course/{id}")
    public ResponseEntity<ArrayList<CourseDao>> getCoursesForStudent(
            @PathVariable Integer departmentId,
            @PathVariable Integer id) {
        return ResponseEntity.ok(studentService.getCoursesForStudent(departmentId, id));
    }

    // Get a student-class by their ID
    @GetMapping("/{departmentId}/student-class/{id}")
    public ResponseEntity<ArrayList<ClassDao>> getClassesForStudent(
            @PathVariable Integer departmentId,
            @PathVariable Integer id) {
        return ResponseEntity.ok(studentService.getClassesForStudent(departmentId, id));
    }

    // Get all students for a given department
    @GetMapping("/{departmentId}/student")
    public ResponseEntity<List<StudentDao>> getStudentsInDepartment(@PathVariable Integer departmentId) {
